...
if not os.getenv("DISABLE_HIGGS_AUDIO_MODEL"):
    ...
    from boson_multimodal.audio_processing.audio_tokenizer import AudioTokenizer
...
class HiggsAudioClient:
    def __init__(self, model, tokenizer, config, whisper_processor, accelerator, audio_tokenizer_type=None, seed=None):
        self.model = model
        self.tokenizer = tokenizer
        self.config = config
        extracted_audio_out_bos_token_id = self.tokenizer.convert_tokens_to_ids("<|audio_out_bos|>")
        extracted_audio_eos_token_id = self.tokenizer.convert_tokens_to_ids("<|audio_eos|>")
        assert isinstance(extracted_audio_out_bos_token_id, int) and isinstance(extracted_audio_eos_token_id, int), f"Special audio tokens may not be in tokenizer as their ids are not integer.: {extracted_audio_out_bos_token_id}, {extracted_audio_eos_token_id}"
        self.model.audio_out_bos_token_id = extracted_audio_out_bos_token_id
        self.model.audio_eos_token_id = extracted_audio_eos_token_id
        ...
        if audio_tokenizer_type is not None:
            try:
                self.audio_tokenizer_model = AudioTokenizer(audio_tokenizer_type, device=accelerator.device)
            except Exception as e:
                self.audio_tokenizer_type = None
                self.audio_tokenizer_model = None
                print(f"Error loading audio tokenizer: {e}")
        if seed is not None:
            print(f"Setting seed for HiggsAudioClient: {seed}")
        self.seed = seed
    ...
    def generate_audio_out(self, system_message, user_message, accelerator, **GENERATION_CONFIG):
        """
        Single turn audio-generation task. optional audio in -> audio out, asr on the audio out.
        """
        if self.audio_tokenizer_type is None or self.audio_tokenizer_model is None:
            raise ValueError("Audio tokenizer is not set, please set it to use audio out.")
        chatml_sample = format_chat_messages(system_message, user_message, None)
        input_tokens, _, _, _ = prepare_chatml_sample(chatml_sample, self.tokenizer)
        assert input_tokens is not None

        # Add assistant prompt without special tokens
        postfix = self.tokenizer.encode("<|start_header_id|>assistant<|end_header_id|>\n\n<|audio_out_bos|>", add_special_tokens=False)
        ...
        batch = {k: (v.contiguous() if isinstance(v, torch.Tensor) else v) for k, v in batch.items()}
        # Generate audio
        do_sample = True if GENERATION_CONFIG["temperature"] > 0 else False
        outputs = self.model.generate(
            **batch,
            max_new_tokens=GENERATION_CONFIG["max_new_tokens"],
            max_audio_new_tokens = GENERATION_CONFIG["max_audio_new_tokens"],
            use_cache=True,
            do_sample=do_sample,
            temperature=GENERATION_CONFIG["temperature"],
            top_p=GENERATION_CONFIG["top_p"],
            ras_win_len=GENERATION_CONFIG.get("ras_win_len", None),
            ras_win_max_num_repeat=GENERATION_CONFIG.get("ras_win_max_num_repeat", 1),
        ...
        audio_out_ids = outputs[1][0]
        wv, decoded_wv_sr = self.audio_tokenizer_model.decode(revert_delay_pattern(audio_out_ids).clip(0, self.audio_tokenizer_model.codebook_size - 1)[:, 1:-1])
        
        # Normalize to 16-bit PCM format
        wv_int16 = np.int16(wv * 32767)

        # Create an audio segment
        pydub_segment = AudioSegment(
            wv_int16.tobytes(),
            frame_rate=decoded_wv_sr,
            sample_width=2,  # 16-bit audio
            channels=1
        )
        return pydub_segment, wv

    def prepare_emergent_tts_sample(self, text_to_synthesize, category, strong_prompting, prompting_object,**kwargs):
        SYSTEM_PROMPT_HIGGS = "You are an AI audio assistant capable of acting as a strong TTS(text-to-speech) system and produce human-like and realistic speech corresponding to a piece of text given by the user."
        if strong_prompting:
        ...
        return SYSTEM_PROMPT_HIGGS, user_message

class MiniCPMClient():
    def __init__(self, model_name, accelerator, *args):
        for _ in range(5):
            try:
    ...
    def generate_audio_out(self, system_message, user_message, accelerator, **GENERATION_CONFIG):
        ...
        audio_np = res["audio_wav"].cpu().numpy()
        ...
        pydub_segment = AudioSegment(
            audio_np.tobytes(),
            frame_rate=res["sampling_rate"],
            sample_width=2,  # 16-bit audio
            channels=1
        )
        return pydub_segment, res
...
class KimiClient():
    def __init__(self, model_name, accelerator, *args):
        raise NotImplementedError("KimiClient did not support TTS when this client was written")
    #     from kimia_infer.api.kimia import KimiAudio
    #     self.model = KimiAudio(model_path=model_name, load_detokenizer=True)
    ...
    #         {"role": "user", "message_type": "text", "content": system_message},
    #         {"role": "user", "message_type": "text", "content": user_message},
    #     ]
    #     res = self.model.generate(messages_conversation, **self.sampling_params, output_type="audio")
...
class BarkClient:
    def __init__(self, model_name, accelerator, voice_to_use):
        ...
        self.model = BarkModel.from_pretrained(model_name, device_map=accelerator.device)
        self.sampling_rate = self.model.generation_config.sample_rate
        self.voice_to_use = voice_to_use if voice_to_use is not None else "v2/en_speaker_6"

    def set_eval_mode(self):
        self.model.eval()

    def generate_audio_out(self, system_message, user_message, accelerator, **GENERATION_CONFIG):
        sentences = nltk.sent_tokenize(user_message)
        silence = np.zeros(int(0.25 * self.sampling_rate))  # quarter second of silence
        pieces = []
        for sentence in sentences:
            inputs = self.processor(sentence, voice_preset = self.voice_to_use)
            for k,v in inputs.items():
                inputs[k] = v.to(accelerator.device)
            audio_array = self.model.generate(**inputs)
            audio_array = audio_array.cpu().squeeze().numpy()
